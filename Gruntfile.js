...
module.exports = function(grunt) {
	...
	grunt.initConfig({
		...
		exec: {
			...
			createDmg: {
				cmd: 'dist/mac/yoursway-create-dmg/create-dmg --volname "Yideo ' + currentVersion + '" --background ./dist/mac/background.png --window-size 480 540 --icon-size 128 --app-drop-link 240 370 --icon "Yideo" 240 110 ./build/releases/Yideo/mac/Yideo-' + currentVersion + '-Mac.dmg ./build/releases/Yideo/mac/'
			},
			createWinInstall: {
		...
		compress: {
			linux32: {
				options: {
					mode: 'tgz',
					archive: 'build/releases/Yideo/linux32/Yideo-' + currentVersion + '-Linux-32.tar.gz'
				},
				expand: true,
				cwd: 'build/releases/Yideo/linux32/Yideo',
				src: '**',
				dest: 'Yideo'
			},
			linux64: {
				options: {
					mode: 'tgz',
					archive: 'build/releases/Yideo/linux64/Yideo-' + currentVersion + '-Linux-64.tar.gz'
				},
				expand: true,
				cwd: 'build/releases/Yideo/linux64/Yideo',
				src: '**',
				dest: 'Yideo'
			},
			windows: {
				options: {
					mode: 'zip',
					archive: 'build/releases/Yideo/win/Yideo-' + currentVersion + '-Win.zip'
				},
				expand: true,
				cwd: 'build/releases/Yideo/win',
				src: 'Yideo-' + currentVersion + '-Win.exe',
				dest: 'Yideo-'+ currentVersion
			}			
		},

		clean: {
			releases: ['build/releases/Yideo/**']
		},
